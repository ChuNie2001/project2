...
data %>% mutate(PC1=pca_1$scores[, 1], PC2=pca_1$scores[, 2]) %>%
  ggplot(aes(PC1, PC2)) + geom_point(aes(color = data$Recency)) + coord_fixed()
```
...
fit <- glm(Response ~ Income+Kidhome+Teenhome+Recency+NumDealsPurchases+NumWebPurchases+NumCatalogPurchases+NumWebVisitsMonth+Complain+Year_Birth, data=data, family="binomial")
...
class_diag(score,data$Response,positive = 1)

table(truth= factor(data$Response==1, levels=c("TRUE","FALSE")),
...
for(i in 1:k){
  ...
  test<-temp[folds==i,]
  truth<-test$Response
  fit<-glm(Response~Income+Kidhome+Teenhome+Recency+NumDealsPurchases+NumWebPurchases+NumCatalogPurchases+NumWebVisitsMonth+Complain+Year_Birth,data=train,family="binomial")
...
knn_fit <- knn3(factor(Response==1,levels=c("TRUE","FALSE")) ~ Income+Kidhome+Teenhome+Recency+NumDealsPurchases+NumWebPurchases+NumCatalogPurchases+NumWebVisitsMonth+Complain+Year_Birth, data=data, k=5)

knn1 <- predict(knn_fit,data)
knn1

class_diag(knn1[,1],data$Response, positive=1)
...
for(i in 1:k){
  ...
  test<-temp[folds==i,]
  truth<-test$Response
  fit<-knn3(Response~Income+Kidhome+Teenhome+Recency+NumDealsPurchases+NumWebPurchases+NumCatalogPurchases+NumWebVisitsMonth+Complain+Year_Birth,data=train)
...
for(i in 1:k){
  train<-temp[folds!=i,]
  test<-temp[folds==i,]
  fit<-lm(Response~.,data=train)
  yhat<-predict(fit,newdata=test)
  diags<-mean((test$Response-yhat)^2)
}
mean(diags)
